...
function useCampusData() {
  const [data, setData] = useState({ students: [], rooms: [], allocations: [] });
  const [state, setState] = useState({ loading: true, error: "" });
  const load = () => {
    setState({ loading: true, error: "" });
    Promise.all([studentService.list(), roomService.list(), allocationService.list()])
      .then(([students, rooms, allocations]) => setData({ students: students.data, rooms: rooms.data, allocations: allocations.data }))
      .catch((error) => setState({ loading: false, error: getErrorMessage(error) }))
...
function StudentHome({ students, rooms, allocations, user }) {
  const student = user.student || students.find((item) => item._id === user.student?._id || item.Rollno === user.student?.Rollno);
  const activeAllocation = allocations.find((allocation) => allocation.status === "Active" && String(allocation.studentId) === String(student?._id));
  const room = rooms.find((item) => String(item._id) === String(activeAllocation?.roomId));
  const roommates = activeAllocation ? allocations.filter((allocation) => allocation.status === "Active" && String(allocation.roomId) === String(activeAllocation.roomId) && String(allocation.studentId) !== String(student?._id)) : [];
  return <div className="page-stack"><div className="page-heading"><div><span className="eyebrow">Resident portal</span><h2>Welcome home, {student?.Name || user.name}</h2><p className="muted">Everything you need to know about your stay, in one view.</p></div></div><div className="student-hero"><div className="student-profile"><div className="profile-avatar">{(student?.Name || user.name).slice(0, 1)}</div><div><span className="eyebrow">Your profile</span><h3>{student?.Name || user.name}</h3><p>{student?.Course || "Course not provided"} · {student?.Campus || "Campus not provided"}</p></div></div><div className="profile-facts"><span><small>Roll number</small><b>{student?.Rollno || "—"}</b></span><span><small>Year</small><b>{student?.Year || "—"}</b></span></div></div>{!activeAllocation ? <div className="panel"><Empty title="No active room allocation" text="Your hostel team has not assigned a room to you yet." /></div> : <div className="content-grid"><section className="panel"><div className="panel-heading"><div><span className="eyebrow">Current allocation</span><h3>Room {room?.RoomNo || activeAllocation.roomNo}</h3></div><Badge>{activeAllocation.status}</Badge></div><div className="detail-grid"><span><small>Block</small><b>{room?.Block || "—"}</b></span><span><small>Floor</small><b>{room?.Floor || "—"}</b></span><span><small>Capacity</small><b>{room?.Capacity || "—"}</b></span><span><small>Occupied students</small><b>{room?.OccupiedCount ?? "—"}</b></span><span><small>Remaining beds</small><b>{room ? Math.max(room.Capacity - room.OccupiedCount, 0) : "—"}</b></span><span><small>Allocation date</small><b>{formatDate(activeAllocation.allocationDate)}</b></span></div></section><section className="panel"><div className="panel-heading"><div><span className="eyebrow">Shared space</span><h3>Roommates</h3></div><span className="panel-count">{roommates.length}</span></div>{roommates.length ? roommates.map((allocation) => <div className="mate-row" key={allocation.id}><span className="avatar small">{allocation.studentName?.slice(0, 1)}</span><span><strong>{allocation.studentName}</strong><small>Active resident</small></span></div>) : <Empty title="No roommates" text="You currently have this room to yourself." />}</section></div>}</div>;
}
